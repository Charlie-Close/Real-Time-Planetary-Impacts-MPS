...
static inline float W_fast(float r, float H1, float fac) {
#ifdef WENDLAND_C2_KERNEL
    float u = r * H1;
    if (u > 1.f) {
        return 0.f;
...
static inline float3 gradW_fast(float3 x_ij, float r, float r1, float H1, float fac) {
#ifdef WENDLAND_C2_KERNEL
    float u = r * H1;
    if (u > 1) {
        return { 0, 0, 0 };
...
static inline float dW_dh_fast(float r, float H1, float fac) {
#ifdef WENDLAND_C2_KERNEL
    float u = r * H1;
    if (u > 1) {
        return 0;
